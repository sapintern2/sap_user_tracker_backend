...
class DeletedUser(Base):
    ...
    deleted_date: Mapped[datetime] = mapped_column(DateTime, nullable=False, index=True)
    last_seen_date: Mapped[datetime] = mapped_column(DateTime, nullable=False)
    ...
    previous_upload = relationship(
        "Upload",
        back_populates="deleted_as_previous",
        foreign_keys=[previous_upload_id],
    )
    current_upload = relationship(
        "Upload",
        back_populates="deleted_as_current",
